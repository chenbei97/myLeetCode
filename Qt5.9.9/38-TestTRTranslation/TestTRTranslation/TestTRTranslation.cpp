...
#include "ui_TestTRTranslation.h"
#include <QMessageBox>
#include <QCloseEvent>
#include <QTranslator>
#include <QSettings>
...
TestTRTranslation::TestTRTranslation(QWidget *parent)
  : QMainWindow(parent)
  , ui(new Ui::TestTRTranslation)
{
    ui->setupUi(this);
...
TestTRTranslation::~TestTRTranslation()
{
    delete ui;
}

void TestTRTranslation::closeEvent(QCloseEvent *event)
{ //窗口关闭时询问是否退出
   QMessageBox::StandardButton result=QMessageBox::question(this, tr("确认"), tr("确定要退出本程序吗？"),//tr封装
...
void TestTRTranslation::setActLocateEnable(bool enable)
{ //设置actTab_Locate的enable属性
    ui->actTab_Locate->setEnabled(enable);
}

void TestTRTranslation::selectACell(int row, int column)
{
...
void TestTRTranslation::setACellText(int row, int column, QString &text)
{//定位到单元格，并设置字符串
...
void TestTRTranslation::on_currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
    Q_UNUSED(previous)
...
void TestTRTranslation::on_actTab_SetSize_triggered()
{ //模态对话框，动态创建，用过后删除
...
void TestTRTranslation::on_actTab_SetHeader_triggered()
...
void TestTRTranslation::on_actTab_Locate_triggered()
...
void TestTRTranslation::on_tableView_clicked(const QModelIndex &index)
