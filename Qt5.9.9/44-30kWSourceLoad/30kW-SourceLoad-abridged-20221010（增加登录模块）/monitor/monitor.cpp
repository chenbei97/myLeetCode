...
void Monitor::setMessageState(TIMEOUTCODE code)
{
      ...
      if (code == TIMEOUTCODE::RequestNormal || code == TIMEOUTCODE::RequestTimeout)
          MessageRequestState->setText(tr("消息请求状态：%1").arg(errorString)); // 请求状态
      else MessageResponseState->setText(tr("消息回复状态：%1").arg(errorString)); // 回复状态
}

void Monitor::setConnectState(SERIALERROR error)
{
      QString errorString = translateErrorCode(error);
      ConnectState->setText(tr("串口连接状态：%1").arg(errorString));
}

void Monitor::setConnectState(TCPERRORSTATE error)
{
      QString errorString = translateErrorCode(error);
      ConnectState->setText(tr("TCP连接状态：%1").arg(errorString));
}
...
void Monitor::showSetState(bool ret, SETTINGVARIABLE role)
{
      ...
      if (ret) MessageSetState->setText(tr("设定状态：%1%2").arg(roleString).arg("设定成功"));
      else MessageSetState->setText(tr("设定状态：%1%2").arg(roleString).arg("设定失败"));
}
