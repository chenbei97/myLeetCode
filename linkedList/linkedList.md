...
可以对链表进行两次遍历，也就是对迭代解法进行空间优化。第一次遍历时，我们统计链表中的元素个数 N；第二次遍历时，我们遍历到第 N/2 个元素时，将该元素返回即可。

## doublePointers解法

可以继续优化单指针方法，用两个指针 slow 与 fast 一起遍历链表。slow 一次走一步，fast 一次走两步。那么当 fast 到达链表的末尾时，slow 必然位于中间。
