...
QHPieModelMapper是饼图系列的水平模型映射器。
模型映射器允许使用从QAbstractItemModel类派生的数据模型作为图表的数据源。水平模型映射器用于在数据模型和QPieSeries之间创建连接，以便数据模型中的每一列定义一个饼图切片，每一行映射到饼图切片的标签或值。
模型和饼图系列财产都可以用于操作数据。模型映射器使饼图系列和数据模型保持同步。

**简单来说就是1行标签，1行数值，列表示一行的指定范围，注意mapping之后会清除model原有的数据。**
...
QVPieModelMapper是饼图系列的垂直模型映射器。
模型映射器允许使用从QAbstractItemModel类派生的数据模型作为图表的数据源。垂直模型映射器用于在数据模型和QPieSeries之间创建连接，以便数据模型中的每一行都定义一个饼图切片，每一列都映射到饼图切片的标签或值。
模型和饼图系列财产都可以用于操作数据。模型映射器使饼图系列和数据模型保持同步。

**简单来说就是1列标签，1列数值，行表示一列的指定范围，注意mapping之后会清除model原有的数据。**
...
允许检索有关卷的空间、装载点、标签和文件系统名称的信息。
您可以通过将到卷的装载点的路径作为构造函数参数来创建QStorageInfo的实例，也可以使用setPath（）方法进行设置。静态mountedVolumes（）方法可用于获取所有已安装文件系统的列表。
QStorageInfo始终缓存检索到的信息，但您可以调用refresh()使缓存无效。
...
/*
rootPath() =  "C:/"   
bytesAvailable() =  27876737024   
bytesFree() =  27876737024   
bytesTotal() =  214749409280   
blockSize() =  -1   
device() =  "\\\\?\\Volume{c8ab65c6-b10b-4010-95e0-0067c7795bbc}\\"   
subvolume() =  ""   
...
rootPath() =  "D:/"   
bytesAvailable() =  269900963840   
bytesFree() =  269900963840   
bytesTotal() =  294972813312   
blockSize() =  -1   
device() =  "\\\\?\\Volume{8a71f394-423a-4c13-adfb-2518130af2e9}\\"   
subvolume() =  ""   
displayName() =  "Data"   
name() =  "Data"   
rootPath() =  "D:/"   
...
rootPath() =  "E:/"   
bytesAvailable() =  236031049728   
bytesFree() =  236031049728   
bytesTotal() =  251652210688   
blockSize() =  -1   
device() =  "\\\\?\\Volume{40a4932d-06a6-11ee-9f01-d4d853f57d45}\\"   
subvolume() =  ""   
displayName() =  "E:/"   
name() =  ""   
rootPath() =  "E:/"   
...
int blockSize() const; // 返回此文件系统的最佳传输块大小
qint64 bytesAvailable() const; // 返回当前用户可用的大小（以字节为单位）
qint64 bytesFree() const;// 返回卷中的可用字节数。请注意，如果文件系统上有配额，则该值可以大于bytesAvailable()返回的值。
qint64 bytesTotal() const;// 返回以字节为单位的总卷大小
QByteArray device() const;// 返回此卷的设备
QByteArray subvolume() const;//返回此卷的子卷名称,允许在一个设备内有多个子卷，可以返回一个空字节数组
QByteArray fileSystemType() const;//
QString displayName() const;
QString name() const;//返回文件系统的可读名称，通常称为标签
QString rootPath() const;//返回代表的文件系统装载点。在Windows上，如果卷未装入目录，它会返回卷号
...
void setPath(const QString &path);//将此QStorageInfo对象设置为路径所在的文件系统。路径可以是文件系统的根路径、目录或该文件系统中的文件
void refresh();//重置QStorageInfo的内部缓存。QStorageInfo缓存有关存储的信息以提高性能

// 返回与当前装载的文件系统列表相对应的QStorageInfo对象列表。Windows，返回“我的电脑”文件夹中可见的驱动器。Unix，返回所有挂载的文件系统（伪文件系统除外）的列表,默认情况下返回所有当前装载的文件系统。
...
QSharedPointer是C++中的一个自动共享指针。它的行为与用于正常目的的正常指针完全相同，包括对常量的尊重。如果没有其他QSharedPointer对象引用它，QSharedPPointer将在指针超出范围时删除它所持有的指针。
可以从普通指针、另一个QSharedPointer对象或通过将QWeakPointer对象提升为强引用来创建QSharedPPointer对象。

**线程安全**

QSharedPointer和QWeakPointer是可重入类。这意味着，通常情况下，在没有同步的情况下，多个线程无法同时访问给定的QSharedPointer或QWeakPointer对象。
多个线程可以同时安全地访问不同的QSharedPointer和QWeakPointer对象。这包括它们持有指向同一对象的指针的情况；引用计数机制是原子的，不需要手动同步。
需要注意的是，虽然指针值可以以这种方式访问（即，由多个线程同时访问，无需同步），但QSharedPointer和QWeakPointer不能保证所指向的对象。该对象的特定线程安全和可重入规则仍然适用。

**其他指针**

Qt还提供了另外两个指针包装类：QPointer和QSharedDataPointer。它们彼此不兼容，因为每个都有非常不同的用例。
QSharedPointer通**过外部引用计数（即放置在对象外部的引用计数器）保存共享指针**。正如其名称所示，指针值在QSharedPointer和QWeakPointer的所有实例之间共享。但是，指针指向的对象的内容不应被视为共享：只有一个对象。因此，**QSharedPointer不提供分离或复制指向对象的方法**。
另一方面，QSharedDataPointer持有指向共享数据（即，从QSharedData派生的类）的指针。它是通过放置在QSharedData基类中的内部引用计数来实现的。因此，这个类可以根据对被保护数据的访问类型进行分离：如果它是一个非常量访问，它会创建一个原子副本以完成操作。
QEexplicitySharedDataPointer是QSharedDataPointer的一个变体，只是它只有在显式调用QEexpliciitySharedDataPointer:：detachment()时才分离（因此得名）。
QScopedPointer只是保存一个指向堆分配对象的指针，并在其析构函数中删除它。当需要对对象进行堆分配和删除时，这个类很有用，但不再需要了。QScopedPointer是轻量级的，它不使用额外的结构或引用计数。
最后，QPointer持有一个指向QObject派生对象的指针，但它做得很弱。QWeakPointer具有相同的功能，但不赞成将其用于该功能。

**可选指针跟踪**

QSharedPointer的一个功能是指针跟踪机制，可以在编译时启用该功能以进行调试。启用后，QSharedPointer会在全局集中注册它跟踪的所有指针。这允许人们发现错误，比如将同一指针分配给两个QSharedPointer对象。
通过在包含QSharedPointer标头之前定义QT_SHAREPOINTER_TRACK_POINTERS宏，可以启用此函数。
即使在没有该功能的情况下编译代码，使用该功能也是安全的。QSharedPointer将确保从跟踪器中删除指针，即使是从未进行指针跟踪的编译代码中删除指针。
但是，请注意，指针跟踪功能对多继承或虚拟继承有限制（即，在两个不同的指针地址可以引用同一对象的情况下）。在这种情况下，如果指针被强制转换为不同的类型，并且其值发生变化，QSharedPointer的指针跟踪机制可能无法检测到被跟踪的对象是否相同。
...
// const_cast、动态、返回qobject、静态强制转换
QSharedPointer<X> constCast() const;
QSharedPointer<X> dynamicCast() const;
QSharedPointer<X> objectCast() const;
QSharedPointer<X> staticCast() const;

QWeakPointer<T> toWeakRef() const;//返回一个弱引用对象，该对象共享此对象引用的指针

T *data() const; //返回此对象引用的指针的值
T *get() const;// 和data()相同,提供此函数是为了与std:：shared_ptr兼容API
```
...
QWeakPointer是C++中指针的自动弱引用。它不能用于直接取消引用指针，但可以用于验证指针是否已在其他上下文中删除。
QWeakPointer对象只能通过QSharedPointer的赋值来创建。
需要注意的是，QWeakPointer没有提供自动强制转换运算符来防止错误的发生。即使QWeakPointer跟踪指针，它本身也不应该被视为指针，因为它不能保证所指向的对象仍然有效。
因此，要访问QWeakPointer正在跟踪的指针，必须首先将其提升为QSharedPointer，并验证生成的对象是否为null。QSharedPointer保证对象不会被删除，所以如果您获得了一个非null对象，您可以使用该指针。有关示例，请参见QWeakPointer:：toStrongRef()
...
QSharedDataPointer<T>使编写自己的隐式共享类变得容易。QSharedDataPointer实现了线程安全引用计数，确保将QSharedData指针添加到可重入类中不会使它们不可重入。
许多Qt类使用隐式共享来将指针的速度和内存效率与类的易用性相结合。有关详细信息，请参阅“共享类”页面。
假设您想使Employee类隐式共享。过程是：将类Employee定义为具有类型为QSharedDataPointer<EmployeeData>的单个数据成员。
定义从QSharedData派生的EmployeeData类，以包含您通常会放入Employee类中的所有数据成员。
为了在实践中展示这一点，我们查看了隐式共享Employee类的源代码。在头文件中，我们定义了Employee和EmployeeData两个类。
 ...
 #include <QSharedData>
  ...
  class Employee
  {
    public:
      ...
      Employee(const Employee &other) //拷贝构造函数 可以隐式共享
            : d (other.d)
      {
      }
    ...
    private:
      QSharedDataPointer<EmployeeData> d;// 数据隐式共享
  };

//隐式共享可能不适合Employee类。考虑一个创建隐式共享Employee类的两个实例的简单示例。
  #include "employee.h"
  int main()
  {
...
员工e2被创建并且e1被分配给它之后，e1和e2指向EmployeeData的同一实例，该实例的引用计数为2。然后调用e1.setName（“Hans-Holbein”）来更改员工名称，但由于引用计数大于1，因此在更改名称之前会执行写时复制。现在，e1和e2指向不同的EmployeeData对象。他们有不同的名字，但都有ID 1001，这可能不是你想要的。当然，如果你真的想创建第二个唯一的雇员，你可以继续使用e1.setId（1002），但如果你只想在任何地方更改雇员的名字，可以考虑在employee类中使用**显式共享**，而不是隐式共享。
如果您将Employee类中的d指针声明为**QEexplicitySharedDataPointer<EmployeeData>，则会使用显式共享**，并且不会自动执行写时复制操作（即，在非常量函数中不会调用detachment()）。在这种情况下，在e1.setName（“Hans-Holbein”）之后，员工的名称已经更改，但e1和e2仍然引用EmployeeData的同一实例，因此只有一个ID为1001的员工。
在成员函数文档中，d指针总是指指向共享数据对象的内部指针。
...
T *data();
const T *data() const;
void detach();//如果共享数据对象的引用计数大于1，则此函数将创建共享数据对象深层副本，并将其d指针设置为该副本。如果需要写时复制，QSharedDataPointer的非常数成员函数会自动调用此函数。你不需要自己称呼它
```

### QExplicitlySharedDataPointer

QEexplicitySharedDataPointer<T>使编写自己的显式共享类变得容易。QEexplicitySharedDataPointer实现线程安全引用计数，确保将QEexplicilySharedDataPointers添加到可重入类不会使它们成为不可重入的。
除了一个很大的区别，QEexplicitySharedDataPointer和QSharedDataPointer一样。**最大的区别在于，QEexplicitySharedDataPointer的成员函数在允许修改共享数据对象之前，不会像QSharedDataPointer中的非常数成员那样执行写时自动复制操作（detach()）**。有一个detach()函数可用，但如果你真的想detach()，你必须自己调用它。这意味着QEexplicitySharedDataPointers的行为类似于常规C++指针，**只是通过执行引用计数并在引用计数为0之前不删除共享数据对象，它们可以避免悬挂指针问题**。
通过一个例子将QEexplicitySharedDataPointer与QSharedDataPointer进行比较是很有启发性的。请考虑QSharedDataPointer中的Employee示例，该示例被修改为使用显式共享，如讨论“隐式与显式共享”中所述。
请注意，如果您使用这个类，但发现您经常调用detach()，那么您可能应该使用QSharedDataPointer。
在成员函数文档中，d指针总是指指向共享数据对象的内部指针。

```c++
QExplicitlySharedDataPointer<Base> base(new Base);
QExplicitlySharedDataPointer<Derived> derived(base); // !!! DANGER !!!这样是不允许的
```
...
手动管理堆分配的对象既困难又容易出错，常见的结果是代码泄漏内存，而且很难维护。QScopedPointer是一个小型实用程序类，它通过将基于堆栈的内存所有权分配给堆分配（通常称为资源获取即初始化（RAII））来大大简化这一过程。QScopedPointer保证当当前作用域消失时，所指向的对象将被删除。
考虑一下这个函数，它执行堆分配，并具有各种退出点：
...
它被手动删除调用所阻碍。使用QScopedPointer，代码可以简化为：
...
保护指针QPointer<T>的行为类似于普通C++指针T*，只是当被引用对象被破坏时会自动清除（不像普通C++指针，在这种情况下会变成“悬挂指针”）。T必须是QObject的子类。
当您需要存储指向其他人拥有的QObject的指针时，保护指针非常有用，因此当您仍然持有对它的引用时，它可能会被销毁。您可以安全地测试指针的有效性。
注意，当使用QPointer时，Qt 5在行为上引入了轻微的变化。
当在QWidget（或QWidget的子类）上使用QPointer时，之前该QPointer将由QWidget析构函数清除。现在，QPointer被QObject析构函数清除（因为这是QWeakPointer对象被清除的时候）。在QWidget析构函数销毁被跟踪的小部件的子对象之前，任何跟踪小部件的QPointer都不会被清除。
Qt还提供了QSharedPointer，这是一个引用计数的共享指针对象的实现，可用于维护对单个指针的引用集合。
...
如果同时删除QLabel，则label变量将包含nullptr而不是无效地址，并且永远不会执行最后一行。
QPointer中可用的函数和运算符与普通无保护指针中可用的相同，但指针算术运算符（+、-、++和-）除外，它们通常仅用于对象数组。
像使用普通指针一样使用QPointer，您将不需要阅读此类文档。
为了创建保护指针，可以从一个T * 或另一个相同类型的保护指针构造或分配给它们。您可以使用operator==()和operator!=()，或使用isNull()测试nullptr。您可以使用* x 或 x->成员表示法来取消对它们的引用。
保护指针将自动转换为T * ，因此您可以自由地混合保护指针和非保护指针。这意味着，如果您有一个QPointer<QWidget>，您可以将其传递给需要QWidget*的函数。因此，将QPointer作为参数来声明函数的价值很小；只需使用普通指针。随着时间的推移存储指针时，请使用QPointer。
