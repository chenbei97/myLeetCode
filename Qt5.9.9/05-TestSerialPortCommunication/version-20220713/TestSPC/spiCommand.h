...
namespace SPIC {
  ...
  typedef enum class spiSenderInfo{
     ...
     POW_EL_SENDER,// 任何模式下功率EL设定发送
      ...
      POW_ON_SENDER,//电源输出设置开启的发送
      POW_OFF_SENDER,//电源输出设置关闭的发送
  ...
  typedef enum class spiCommand
  {
        ...
        // 优先CR模式电源内阻当前值、最小、最大和默认值查询和设置（暂时不支持）
...
//        {COMMAND::SOUR_RES_GET_DEF,"SOUR:RES? DEF"},    {COMMAND::SOUR_RES_SET_DEF,"SOUR:RES DEF"},
        ...
        // STATus子系统
        {COMMAND::STAT_OPERA_GET,"STAT:OPER?"}, // 电源工作状态

        // OUTPut子系统
        {COMMAND::OUTP_GET_STATE,"OUTP?"}, // 询问、开启和禁用电源输出
        {COMMAND::OUTP_SET_ON_STATE,"OUTP ON"},
        {COMMAND::OUTP_SET_OFF_STATE,"OUTP OFF"},
        ...
        {COMMAND::GET_WORK_STATUS,"STAT:OPER?"},  // 等价于STATUS_OPERA_GET,相当于是别名
  };
  inline static const QString& spic(const COMMAND& spi)
  {
    return SPICS[spi];
  }
} // END NAMESPACE SPIC
