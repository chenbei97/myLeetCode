...
QPicture类是一个绘制设备，用于记录和重放QPaint命令。
...
QPainter是一个画家，他拿着画笔QPen控制线条属性（例如线条颜色、宽度和线型），拿着颜料桶QBrush控制填充属性（填充颜色、填充方式和渐变特性），辅助工具QFont控制文字格式，QColor控制颜色。**QPainter 可以对任何继承 QPaintDevice 类的对象进行操作**。常见用途是在小部件的绘制事件中：例如：
...
QPainter 提供了 CompositionMode 枚举，它定义了数字图像合成的 Porter-Duff 规则；它描述了一个模型，用于将一个图像（源）中的像素与另一图像（目标）中的像素组合起来。两种最常见的合成形式是 Source 和 SourceOver。源用于将不透明对象绘制到绘图设备上。在这种模式下，源中的每个像素都会替换目标中的相应像素。在 SourceOver 合成模式下，源对象是透明的并绘制在目标之上。部分模式的合成效果如下图所示。
...
![QPainterTransform](C:/Users/Lenovo/Desktop/myLeetCode/Qt5.9.9/QPainterTransform.jpg)
...
QAbstractGraphicsShapeItem类为所有路径项提供公共基础。
此类本身并不能完全实现项；特别是，它不实现由QGraphicsItem继承的boundingRect()和paint()。
您可以对该项进行子类化，为该项的画笔提供一个简单的访问器的基本实现。
...
void setPolygon(const QPolygonF &polyon);
```

##### QGraphicsObject

QGraphicsObject类为所有需要信号、插槽和属性的图形项提供基类。
该类使用QObject的信号/槽和属性机制扩展QGraphicsItem。它将QGraphicsItem的许多基本setter和getter映射到属性，并为其中许多添加通知信号。
父项和子项每个图形对象都可以用父项构造。这样可以确保在销毁其父项时销毁该项。虽然QGraphicsObject继承自QObject和QGraphicsItem，但您应该使用QGraphicsIItem而不是QObject提供的函数来管理父项和子项之间的关系。可以使用parentItem()和childItems()函数探索项之间的关系。在场景中的项目层次结构中，parentObject()和parentWidget()函数与QWidget子类的QWidget::parent()和QWidget::parentWidget()函数等效。
...
void ungrabGesture(Qt::GestureType gesture);//从给定手势中取消订阅图形对象
```

信号和槽函数。

```c++
//图形项的使能、透明度、父容器、角度、缩放、可见性以及三个坐标方向移动信号发射
void enabledChanged();
void opacityChanged();
void parentChanged();
...
#### 7.3.4 QGraphicsLayoutItem

可以继承QGraphicsLayoutItem类，以允许通过布局管理自定义项。
QGraphicsLayoutItem是一个抽象类，它定义了一组虚拟函数，这些函数描述了QGraphicsLayout排列的任何对象的大小、大小策略和大小提示。API包含与项本身和项用户相关的函数，因为QGraphicsLayoutItem的大多数函数也是子类公共API的一部分。在大多数情况下，现有的布局感知类（如QGraphicsWidget和QGraphicsLayout）已经提供了所需的功能。然而，对这些类进行子类化将使您能够创建与布局（QGraphicsWidget）或自定义布局（QGraphicsLayout）配合良好的图形元素。

如果您创建了QGraphicsLayoutItem的子类并重新实现了其虚拟函数，那么您将使布局能够调整项目的大小并将其与其他QGraphicsLayoutItem（包括QGraphicsWidget和QGraphicsLayout）一起定位。
您可以首先重新实现重要的函数：受保护的sizeHint()函数，以及公共的setGeometry()函数。如果您想让项目知道几何体的即时变化，还可以重新实现updateGeometry()。
几何图形、大小提示和大小策略会影响项目的大小和位置。调用setGeometry()将始终立即调整项目大小并重新定位。通常，在布局被激活后，QGraphicsLayout会调用此函数，但项目的用户也可以随时调用此函数。
sizeHint()函数返回项的最小、首选和最大大小提示。您可以通过调用setMinimumSize()、setPreferredSize()或setMaximumSize()来覆盖这些属性。如果需要，还可以使用setMinimumWidth()或setMaximumHeight()等函数来仅设置宽度或高度组件。
另一方面，validateSizeHint()函数为任何给定的Qt::SizeHin返回大小提示，并保证返回的大小与最小和最大大小以及大小提示绑定。您可以通过调用setSizePolicy()来设置项目的垂直和水平大小策略。布局系统使用sizePolicy属性来描述此项的增长或收缩方式。

QGraphicsLayoutItems可以嵌套在其他QGraphicsLayoutItems中，类似于可以包含子布局的布局。这可以通过将QGraphicsLayoutItem指针传递给QGraphicsLayoutItem的受保护构造函数或调用setParentLayoutItem()来完成。parentLayoutItem()函数返回指向项的layoutItem父级的指针。如果项的父项为0，或者父项未从QGraphicsItem继承，则parentLayoutItem()函数将返回0。如果QGraphicsLayoutItem子类本身是布局，则isLayout()返回true，否则返回false。
Qt使用QGraphicsLayoutItem在图形视图框架中提供布局功能，但未来它的使用可能会扩展到Qt本身。

##### QGraphicsLayout

QGraphicsLayout类为图形视图中的所有布局提供基类。
QGraphicsLayout是一个抽象类，它定义了一个虚拟API，用于安排QGraphicsWidget的子级和其他QGraphicsLayoutItem对象。QGraphicsWidget通过QGraphicsWidget::setLayout()将责任分配给QGraphicsLayout。当小部件调整大小时，布局将自动排列小部件的子部件。QGraphicsLayout继承了QGraphicsLayoutItem，因此它可以由任何布局管理，包括它自己的子类。

您可以使用QGraphicsLayout作为基础来编写自己的自定义布局（例如，流布局），但更常见的是使用其子类之一QGraphicsLinearLayout或QGraphicsGridLayout。创建自定义布局时，必须至少重新实现以下功能：

QGraphicsLayoutItem::setGeometry()) 在设置布局的几何图形时通知您,将几何图形存储在自己的布局类中。
QGraphicsLayoutItem::sizeHint()返回布局的大小提示。
QGraphicsLayout::count()返回布局中的项数。
QGraphicsLayout::itemAt()返回指向布局中项目的指针。
QGraphicsLayout::removeAt()从布局中删除项而不销毁它。

有关如何实现每个函数的更多详细信息，请参阅单个函数文档。
每个布局都定义了自己的API，用于排列小部件和布局项。例如，对于网格布局，需要具有可选行和列跨度、对齐方式、间距等的行和列索引。然而，线性布局需要单个行或列索引来定位其项。对于栅格布局，插入顺序不会以任何方式影响布局，但对于线性布局，顺序至关重要。在编写自己的布局子类时，您可以自由选择最适合您的布局的API。
QGraphicsLayout提供了addChildLayoutItem()方便函数，用于将布局项添加到自定义布局中。如果需要，该功能将自动修复图形项。

当布局的几何图形发生变化时，QGraphicsLayout通过对每个项调用setGeometry()立即重新排列其所有托管项。这种重新排列称为激活布局。QGraphicsLayout更新其自身的几何图形，以匹配其管理的QGraphicsLayoutItem的contentsRect()。因此，当小部件调整大小时，它将自动重新排列所有项目。QGraphicsLayout缓存其所有托管项的大小，以避免过于频繁地调用setGeometry()。
注意：QGraphicsLayout将具有与其分配给的小部件（而不是布局）的contentsRect()相同的几何图形。

可以使用以下两种方法之一隐式激活布局：调用activate()或调用invalidate()。调用activate()将立即激活布局。相反，调用invalidate()会延迟，因为它会向托管小部件发布LayoutRequest事件。由于事件压缩，在控件返回到事件循环后，activate()将仅被调用一次。这被称为使布局无效。使布局无效也会使任何缓存的信息无效。此外，invalidate()函数是一个虚拟函数。因此，您可以通过重新实现此函数来使QGraphicsLayout子类中的缓存无效。

QGraphicsLayout通过虚拟widgetEvent()事件侦听它管理的小部件的事件。当布局分配给一个小部件时，传递给该小部件的所有事件首先由widgetEvent()处理。这允许布局了解小部件上的任何状态更改，例如可见性更改或布局方向更改。
...
~QGraphicsLayout();
void activate();
bool isActivated() const;//激活布局，使布局中的所有项目立即重新排列
virtual int count() const = 0;//返回布局中的项数
virtual void invalidate();//清除布局中任何缓存的几何图形和大小提示信息，并将LayoutRequest事件发布到托管的父级QGraphicsLayoutItem
virtual QGraphicsLayoutItem *itemAt(int i) const = 0;
virtual void removeAt(int index) = 0;//删除索引处的项
void setContentsMargins(qreal left, qreal top, qreal right, qreal bottom);//内容边距设置
virtual void widgetEvent(QEvent *e);//此虚拟事件处理程序接收托管小部件的所有事件。QGraphicsLayout使用此事件处理程序侦听与布局相关的事件，例如几何图形更改、布局更改或布局方向更改
```

###### QGraphicsLinearLayout

QGraphicsLinearLayout类为管理图形视图中的小部件提供了水平或垂直布局。
线性布局的默认方向为Qt:：水平。您可以通过调用setOrientation()或将Qt::vertical传递给QGraphicsLinearLayout的构造函数来选择垂直方向。
使用QGraphicsLinearLayout最常见的方法是在堆上构造一个没有父对象的对象，通过调用addItem()添加小部件和布局，最后通过调用QGraphicsWidget::setLayout()将布局分配给小部件。
...
您可以向线性布局中添加小部件、布局、拉伸(addStretch()、insertStretch()或setStretchFactor()和setItemSpacing())。布局获取项目的所有权。在某些情况下，当布局项也继承自QGraphicsItem（例如QGraphicsWidget）时，所有权将存在歧义，因为布局项属于两个所有权层次结构。请参阅QGraphicsLayoutItem:：setOwnedByLayout()的文档，了解如何处理此问题。您可以通过调用count()和itemAt()来访问布局中的每个项。调用removeAt()或removeItem()将从布局中删除项，而不会破坏它。

QGraphicsLinearLayout尊重每个项目的大小提示和大小策略，当布局包含的空间超过项目可以填充的空间时，每个项目都会根据该项目的布局对齐方式进行排列。您可以通过调用setAlignment()为每个项目设置对齐方式，并通过调用alignment()检查任何项目的对齐方式。默认情况下，项目对齐到左上角。

在项目之间，布局分配了一些空间。实际的空间大小取决于托管小部件的当前样式，常用间距为4。可以调用setSpacing()设置自己的间距，并通过调用spacing()获取当前间距值。如果要为项目配置单独的间距，可以调用setItemSpacing()。

您可以为每个项目指定拉伸因子，以控制与其他项目相比将获得多少空间。默认情况下，以线性布局排列的两个相同的小部件将具有相同的大小，但如果第一个小部件的拉伸因子为1，而第二个小部件具有拉伸因子为2，则第一个小组件将获得1/3的可用空间，第二个将获得2/3的可用空间。
QGraphicsLinearLayout通过将所有项目的拉伸因子相加，然后相应地划分可用空间来计算大小分布。所有项目的默认拉伸系数为0；因子为0表示该项没有任何定义的拉伸因子；实际上，这与将拉伸因子设置为1相同。拉伸系数仅适用于布局长度方向上的可用空间（沿其方向）。如果您想控制项目的水平和垂直拉伸，可以改用QGraphicsGridLayout。
...
QGraphicsGridLayout类提供了一个用于在图形视图中管理小部件的网格布局。
使用QGraphicsGridLayout最常见的方法是在堆上构造一个没有父对象的对象，通过调用addItem()添加小部件和布局，最后通过调用QGraphicsWidget::setLayout()将布局分配给小部件。QGraphicsGridLayout在添加项时自动计算网格的尺寸。
...
布局获取项目的所有权。在某些情况下，当布局项也继承自QGraphicsItem（例如QGraphicsWidget）时，所有权将存在歧义，因为布局项属于两个所有权层次结构。请参阅QGraphicsLayoutItem:：setOwnedByLayout()的文档，了解如何处理此问题。您可以通过调用count()和itemAt()来访问布局中的每个项。调用removeAt()将从布局中删除项，而不会破坏它。

QGraphicsGridLayout尊重每个项的大小提示和大小策略，当网格中的单元格的空间超过了项可以填充的空间时，每个项都会根据该项的布局对齐方式进行排列。您可以通过调用setAlignment()为每个项目设置对齐方式，并通过调用alignment()检查任何项目的对齐方式。还可以通过分别调用setRowAlignment()和setColumnAlignment()来设置整行或整列的对齐方式。默认情况下，项目对齐到左上角。
...
QGraphicsAnchorLayout类提供了一种布局，可以在图形视图中将小部件固定在一起。
锚定布局允许开发人员指定小部件应该如何相对于彼此以及布局本身放置。该规范是通过调用addAnchors()、addAnchores()或addCornerAnchors()向布局添加锚点来实现的。可以使用anchor()函数访问布局中的现有锚点。锚定的项目将自动添加到布局中，如果项目被删除，则其所有锚定将自动删除。

锚点始终设置在项目的边缘之间，其中“中心”也被视为边缘。考虑以下示例：
...
这里，项目a的右边缘锚定到项目b的左边缘，而项目a的下边缘锚定在项目b的上边缘，因此项目b将对角放置在项目b右侧和下方。addCornerAnchors()函数提供了一种更简单的方法来锚定两个小部件的角，而不是上面代码中显示的两个单独的addAnchor()调用。在这里，我们可以看到如何将小部件锚定到封闭布局的左上角：

```c++
layout->addCornerAnchors(a, Qt::TopLeftCorner, layout, Qt::TopLeftCorner);
```

在使用锚匹配小部件的宽度或高度的情况下，使用addAnchors()函数很方便。与其他指定锚点的功能一样，它也可以用于将小部件锚定到布局。QGraphicsAnchorLayout尊重每个项目的大小提示和大小策略。请注意，QSizePolicy的某些属性不受尊重。布局可能会在项目之间分配一些空间。如果未明确指定间距，则实际空间量通常为0。然而，如果第一边缘与第二边缘相反(例如，第一小部件的右边缘锚定到第二小部件的左边缘)，则将通过像素度量从样式中查询锚的大小：水平锚的PM_LayoutHorizontalSpacing和垂直锚的PM.LayoutVerticalSpacing。如果间距为负，则项目将在一定程度上重叠。

QGraphicsAnchorLayout当前不支持某些功能。这可能会在未来发生变化，所以如果你想避免未来的行为倒退，请避免使用这些功能：拉伸因素不受尊重。QSizePolicy:：ExpandFlag不受尊重。不考虑宽度的高度。
...
QGraphicsWidget类是QGraphicsScene中所有小部件项的基类。
QGraphicsWidget是一个扩展的基本项，它提供了QGraphicsItem之外的额外功能。它在许多方面与QWidget相似：提供调色板、字体和样式。具有定义的geometry()。支持带有setLayout()和layout()的布局。
支持grabShortcut()和insertAction()的快捷方式和操作。与QGraphicsItem不同，QGraphicsWidget不是抽象类；您可以创建QGraphicsWidget的实例，而不必将其子类化。这种方法对于仅用于将子部件组织到布局中的部件非常有用。
如果需要高级输入焦点处理（例如，选项卡焦点和激活或布局），QGraphicsWidget可用作您自己的自定义项的基本项。由于QGraphicsWidget类似于QWidget，并且具有类似的API，因此将小部件从QWidget移植到QGraphicsWidget而不是QGraphicsItem更容易。注意：基于QWidget的小部件可以使用QGraphicsProxyWidget直接嵌入到QGraphicsScene中。
...
QGraphicsProxyWidget将基于QWidget的小部件（例如，QPushButton、QFontComboBox甚至QFileDialog）嵌入QGraphicsScene中。它在两个对象之间转发事件，并在QWidget的基于整数的几何图形和QGraphicsWidget的基于qreal的几何图形之间进行转换。QGraphicsProxyWidget支持QWidget的所有核心功能，包括选项卡焦点、键盘输入、拖放和弹出窗口。您还可以嵌入复杂的小部件，例如，带有子小部件的小部件。
...
QGraphicsProxyWidget通过为每个弹出窗口创建子代理来自动嵌入嵌入控件的弹出窗口子项。这意味着当嵌入的QComboBox显示其弹出列表时，会自动创建一个新的QGraphicsProxyWidget，嵌入弹出窗口并正确定位。仅当弹出窗口是嵌入式小部件的子项时（例如，QToolButton::setMenu()要求QMenu实例是QToolButton的子项），这才有效。

有两种方法可以使用QGraphicsProxyWidget嵌入小部件。最常见的方法是将小部件指针与任何相关的Qt::WindowFlags一起传递到QGraphicsScene:：addWidget()。此函数返回指向QGraphicsProxyWidget的指针。然后，您可以选择重新分配或定位代理或嵌入式小部件本身。例如，在下面的代码段中，我们在代理中嵌入了一个组框：
...
或者，您可以先创建一个新的QGraphicsProxyWidget项，然后调用setWidget()来嵌入QWidget。函数的作用是返回一个指向嵌入式小部件的指针。QGraphicsProxyWidget与QWidget共享所有权，因此如果这两个小部件中的任何一个被销毁，另一个小部件也将被自动销毁。

QGraphicsProxyWidget保持其状态与嵌入式小部件同步。例如，如果代理被隐藏或禁用，嵌入的小部件也将被隐藏或停用，反之亦然。当通过调用addWidget()嵌入小部件时，QGraphicsProxyWidget将状态从小部件复制到代理中，之后，两者将尽可能保持同步。默认情况下，当您将小部件嵌入到代理中时，小部件和代理都是可见的，因为QGraphicsWidget在创建时是可见的(您不必调用show())。如果显式隐藏嵌入的小部件，代理也将变得不可见。
...
与图表相关的类有QAbstractAxis(QValueAxis)、QAbstractSeries(QLineSeries)、QEasingCurve、QLegend、QLocale、AnimationOptions。关于这些抽象类和具体类的介绍后边会继续给出
...
QAbstractSeries-->QCandlestickSeries
...
QCandlerstickSeries类将数据显示为烛台。
此类充当单个烛台项目的容器。使用QBarCategoryAxis时，每个项目都被绘制到自己的类别。QDateTimeAxis和QValueAxis可以用作QBarCategoryAxis的替代品。在这种情况下，每个烛台项目都根据其时间戳值绘制。
注意：时间戳在QCandlerstickSeries中必须是唯一的。使用QBarCategoryAxis时，只绘制共享时间戳的第一个烛台项。如果图表包含QCandlerstickSeries的多个实例，则来自共享时间戳的不同系列的项目将被绘制到同一类别。使用QValueAxis或QDateTimeAxis时，共享时间戳的烛台项目将彼此重叠。

成员函数。

```c++
bool append(QCandlestickSet *set);//添加取出插入移除返回烛台项
...
bool bodyOutlineVisible() const;//烛台主体轮廓的可见性
qreal bodyWidth() const;//烛台项目在其自身插槽中的相对宽度，范围从0.0到1.0

void setCapsVisible(bool capsVisible);//帽子的可见性和宽度
...
void clear();
int count() const;//一系列烛台的数量

void setDecreasingColor(const QColor &decreasingColor);//逐渐减少增加的烛台物品主体的颜色
...
qreal maximumColumnWidth() const;//烛台项目的最大/小宽度
...
```c++
virtual QAbstractSeries *series() = 0;//返回与此图例标记相关的曲线指针。图例标记始终与系列相关
...
QXYLegendMarker类是**直线、样条曲线或散点系列**的图例标记。
XY图例标记与QXYSeries派生类相关：QLineSeries、QSplineSeries和QScatterSeries。每个标记都与一个系列相关。

##### QAreaLegendMarker

QAreaLegendMarker类是**面积系列**的图例标记。
区域图例标记与QAreaSeries对象相关，因此一个区域系列产生一个标记。

##### QBarLegendMarker

QBarLegendMarker类是**条形图系列**的图例标记。
条形图例标记与QAbstractBarSeries派生类相关。对于条形图系列，每个标记都与一个QBarSet相关。
...
QBoxPlotLegendMarker类是**方框图系列**的图例标记。
方框图图例标记与QBoxPlotSeries对象相关。

##### QCandlestickLegendMarker

QCandlestickLegendMarker类是**烛台系列**的图例标记。
QCandlestickLegendMarker与QCandlesstickSeries相关，因此一个烛台系列产生一个标记。

##### QPieLegendMarker

QPieLegendMarker类是**饼图系列**的图例标记。
饼图图例标记与QPieSeries相关。对于饼图系列，饼图的每个切片都与图例中的一个标记相关。
...
#### 8.3.5 QCandlestickSet

QCandlerstickSet类表示烛台图表中的单个烛台项目。
创建烛台项目的图形表示需要五个值：**打开、高、低、关闭和时间戳**。这些值可以传递给QCandlerstickSet构造函数，也可以通过使用setOpen()、setHigh()、setLow()、set Close()和setTimestamp()进行设置。
...
QSqlDriverCreatorBase SQL驱动程序厂的基类
QSqlDriverPlugin 自定义QSql驱动程序插件
QSqlError SQL数据库错误信息
QSqlField 操作SQL数据库表和视图中的字段
QSqlIndex 操作和描述数据库索引的函数方法
QSqlQuery 执行和操作SQL语句
QSqlQueryModel SQL结果集的只读数据模型
QSqlRecord 封装数据库记录
QSqlRelation 存储有关SQL外键Delegate的信息
QSqlRelationalDelegate 该外键用于显示和编辑来自QSqlRelationalTableModel的数据
QSqlRelationalTableModel 单个数据库表的可编辑数据模型
QSqlResult 具有外键支持用于访问特定SQL数据库中的数据的抽象接口
QSqlTableModel 单个数据库表的可编辑数据模型

### 10.1 QSqlDatabase

数据库基于驱动或者打开，驱动支持**"QSQLITE","QMYSQL","QOCI"**等或者自定义的驱动，关联类QSqlDriver。

一般使用静态函数addDatabase来打开数据库，对数据库可以设置名称、用户名和密码、主机名称和端口、连接选项(不同数据库有不同选项)，之后就可以使用open来打开数据库。
...
bool rollback();// 回滚事务
bool open();
bool open(const QString &user, const QString &password);//如果打开数据库连接时出错，则返回true；否则返回false。可以使用lastError()函数检索错误信息。
bool transaction();//如果驱动程序支持事务，则在数据库上开始事务。如果操作成功返回true否则返回false
...
QStringList tables(QSql::TableType type = QSql::Tables) const;//返回表格列表
int port() const;
...
QSqlIndex primaryIndex(const QString &tablename) const;//返回表的主索引
QSqlRecord record(const QString &tablename) const;// 返回表的字段元信息
...
// 设置精度 LowPrecisionInt32、LowPrecisionInt64、LowPrecisionDouble、HighPrecision
...
// 如果数据库连接列表中包含connectionName，则返回true；否则返回false
static bool contains(const QString &connectionName = QLatin1String(defaultConnection))
static bool isDriverAvailable(const QString &name);//检查是否有特定的驱动程序可用
// 注册定制驱动程序
...
QSqlError对象可以提供特定于数据库的错误数据，包括driverText()和databaseText()消息（或两者连接为text()），以及nativeErrorCode()和type()。
...
```c++
enum RequiredStatus { Required, Optional, Unknown };//插入记录时必须指定字段,可选字段。
QSqlField::RequiredStatus requiredStatus() const;
void setRequiredStatus(QSqlField::RequiredStatus required);

void clear();

bool isAutoValue() const;//如果该值是由数据库自动生成的，则返回true，例如自动递增主键值
bool isGenerated() const;//如果生成了字段，则返回true；否则返回false
bool isNull() const;
...
QSqlField表示数据库表或视图中单个列的特征，例如数据类型和列名。字段还包含数据库列的值，可以查看或更改该值。字段数据值存储为QVariants。不允许使用不兼容的类型。例如：
...
QSqlField对象很少在应用程序代码中显式创建。它们通常是通过已经包含字段列表的QSqlRecords间接访问的。例如：
...
QSqlField对象可以提供有关字段的一些元数据，例如，它的名称()、变量类型()、长度()、精度()、默认值()、类型typeID()以及它的requiredStatus()、isGenerated()和isReadOnly()。可以检查字段的数据以查看它是否为isNull()，并检索其值value()。编辑数据时，可以使用setValue()设置数据，也可以使用clear()设置为NULL。

### 10.4 QSqlRecord

QSqlRecord类封装了数据库记录（通常是数据库中表或视图中的一行）的功能和特征。QSqlRecord支持添加和删除字段以及设置和检索字段值。
记录字段的值可以使用setValue()按名称或位置设置；如果要将字段设置为null，请使用setNull()。要按名称查找字段的位置，请使用indexOf()，要在特定位置查找字段的名称，请使用fieldName()。使用field()检索给定字段的QSqlField对象。使用contains()查看记录是否包含特定的字段名。
当生成要在数据库上执行的查询时，生成的SQL中只包括isGenerated()为true的那些字段。
一条记录可以用append()或insert()添加字段，用replace()替换字段，用remove()移除字段。所有字段都可以用clear()删除。字段的数量由count()给出；它们的所有值都可以使用clearValues()清除为null。
...
bool isEmpty() const；
bool isGenerated(const QString &name) const；//如果记录有一个名为name的字段并且要生成此字段（默认值），则返回true；否则返回false
bool isGenerated(int index) const；
bool isNull(const QString &name) const；
bool isNull(int index) const；

int count() const；
int indexOf(const QString &name) const；//返回名为name的字段在记录中的位置，如果找不到，则返回-1。字段名不区分大小写。如果有多个字段匹配，则返回第一个字段

void setGenerated(const QString &name, bool generated)；//将名为name的字段的已生成标志设置为已生成。如果字段不存在，则不会发生任何事情。例如，只有已生成并设置为true的字段才会包含在由QSqlQueryModel生成的SQL中。
...
QSqlRecord keyValues(const QSqlRecord &keyFields) const；//返回一条记录，该记录包含keyFields中表示的字段，这些字段设置为按字段名称匹配的值
```

### 10.5 QSqlIndex

索引是指数据库中的单个表或视图。关于构成索引的字段的信息可以用于生成SQL语句。

```c++
QSqlIndex(const QString &cursorname = QString(), const QString &name = QString());

void append(const QSqlField &field);//将字段字段追加到索引字段列表中。该字段以升序追加
void append(const QSqlField &field, bool desc);
bool isDescending(int i) const;
QString name() const;
QString cursorName() const;
void setCursorName(const QString &cursorName);//将与索引关联的光标的名称设置为cursorName
void setDescending(int i, bool desc);//如果desc为true，则字段i按降序排序。否则，字段i按升序排序（默认值）。如果字段不存在，则不会发生任何事情
void setName(const QString &name);//将索引的名称设置为name
```

### 10.6 QSqlQuery

QSqlQuery封装了从在QSqlDatabase上执行的SQL查询中创建、导航和检索数据所涉及的功能。它可以用于执行DML（数据操作语言）语句，如**SELECT、INSERT、UPDATE和DELETE，以及DDL**（数据定义语言）语句（如CREATE TABLE）。它还可以用于执行非标准SQL的数据库特定命令（例如，SET DATESTYLE=ISO for PostgreSQL）。
成功执行的SQL语句将查询的状态设置为活动，以便isActive()返回true。否则，查询的状态将设置为非活动状态。在任何一种情况下，当执行新的SQL语句时，查询都位于无效记录上。在检索值之前，必须将活动查询导航到有效记录（以便isValid()返回true）。

对于某些数据库，如果在调用commit()或rollback()时存在作为SELECT语句的活动查询，则提交或回滚将失败。有关详细信息，请参见isActive()。

导航记录的功能如下：next(),previous(),first(),last(),seek()

这些函数允许程序员在查询返回的记录中向前、向后或任意移动。如果您只需要在结果中前进（例如，使用next()），则可以使用setForwardOnly()，这将节省大量内存开销，并提高某些数据库的性能。一旦活动查询位于有效记录上，就可以使用value()检索数据。所有数据都是使用QVariants从SQL后端传输的。
...
若要访问查询返回的数据，请使用值（int）。SELECT语句返回的数据中的每个字段都是通过传递字段在语句中的位置来访问的，从0开始。这使得不建议使用SELECT*查询，因为返回的字段的顺序是不确定的。
为了提高效率，没有按名称访问字段的函数（除非您使用带名称的预处理查询，如下所述）。要将字段名转换为索引，请使用record().indexOf()，例如：
...
QSqlQuery支持准备好的查询执行和参数值到占位符的绑定。有些数据库不支持这些功能，所以对于这些功能，Qt模拟所需的功能。例如，Oracle和ODBC驱动程序有适当的预处理查询支持，Qt利用了它；但对于没有这种支持的数据库，Qt实现了该功能本身，例如在执行查询时用实际值替换占位符。使用numRowsAffected()可以找出有多少行受到非SELECT查询的影响，使用size()可以找到SELECT检索到的行数。
Oracle数据库通过使用冒号名称语法来识别占位符，例如：name。ODBC简单地使用？字符。Qt支持这两种语法，但有一个限制，即不能将它们混合在同一个查询中。
您可以使用boundValues()检索单个变量（映射）中所有字段的值。

下面，我们将使用四种不同的绑定方法中的每一种来提供相同的示例，以及将值绑定到存储过程的一个示例。

```c++
QSqlQuery query;
query.prepare("INSERT INTO person (id, forename, surname) "
              "VALUES (:id, :forename, :surname)"); // ① 使用名称占位符号法
...
query.bindValue(0, 1001); // ② 使用位置占位也可
...
query.prepare("INSERT INTO person (id, forename, surname) "
              "VALUES (?, ?, ?)");
query.addBindValue(1001); // ④ 可以不要位置参数,使用addBindValue
query.addBindValue("Bart");
query.addBindValue("Simpson");
query.exec();
```

这段代码调用一个名为AsciiToInt（）的存储过程，通过其in参数向其传递一个字符，并在out参数中获取其结果。

```c++
QSqlQuery query;
query.prepare("CALL AsciiToInt(?, ?)");
query.bindValue(0, "A");// A对应65,0对应48 但是A以In参数绑定的不替换数据库的数据
query.bindValue(1, 0, QSql::Out);// 0会替换
query.exec();
...
enum QSqlQuery::BatchExecutionMode { 
    QSqlQuery::ValuesAsRows // -更新多行。将QVariantList中的每个条目视为更新下一行的值。
...
void bindValue(const QString &placeholder, const QVariant &val, QSql::ParamType paramType = QSql::In);//使用位置值绑定时，将值val添加到值列表中。addBindValue()调用的顺序决定了值将在准备好的查询中绑定到哪个占位符。如果paramType是QSql:：Out或QSql:：InOut，则在exec（）调用后，占位符将被数据库中的数据覆盖。
void bindValue(int pos, const QVariant &val, QSql::ParamType paramType = QSql::In)
QVariant boundValue(const QString &placeholder) const;
QVariant boundValue(int pos) const;

QMap<QString, QVariant> boundValues() const;//返回绑定值的映射


void clear();
void finish();//将查询设置为非活动。绑定值保留其值
void setForwardOnly(bool forward);
void setNumericalPrecisionPolicy(QSql::NumericalPrecisionPolicy precisionPolicy);

bool exec(const QString &query);//对于SQLite，查询字符串一次只能包含一条语句。如果给定了多个语句，则函数将返回false
bool exec();//执行以前准备的SQL查询。如果查询执行成功，则返回true；否则返回false
bool execBatch(QSqlQuery::BatchExecutionMode mode = ValuesAsRows);//在批处理中执行以前准备的SQL查询。所有绑定参数都必须是变体列表。如果数据库不支持批处理执行，驱动程序将使用传统的exec调用来模拟它
bool first();//检索结果中的第一条记录（如果可用），并将查询定位在检索到的记录上。请注意，结果必须处于活动状态，并且isSelect（）在调用此函数之前必须返回true，否则将不执行任何操作并返回false。如果成功，则返回true。如果不成功，则将查询位置设置为无效位置，并返回false。
bool isActive() const;
bool isForwardOnly() const;//如果只能在结果集中向前滚动，则返回true；否则返回false
bool isNull(int field) const;
bool isNull(const QString &name) const;
bool isSelect() const;//如果当前查询是SELECT语句，则返回true；否则返回false
bool isValid() const;
bool last();
bool next();//检索结果中的下一条记录（如果可用），并将查询定位在检索到的记录上。请注意，结果必须处于活动状态，并且isSelect（）在调用此函数之前必须返回true，否则将不执行任何操作并返回false
bool nextResult();//放弃当前结果集并导航到下一个结果集（如果可用）
bool prepare(const QString &query);//准备SQL查询查询以供执行。如果查询准备成功，则返回true；否则返回false
bool previous();//检索结果中的上一条记录（如果可用），并将查询定位在检索到的记录上。请注意，结果必须处于活动状态，并且isSelect（）在调用此函数之前必须返回true，否则将不执行任何操作并返回false。
bool seek(int index, bool relative = false);// 检索位置索引处的记录（如果可用），并在检索到的记录上定位查询。第一条记录位于位置0。请注意，查询必须处于活动状态，并且在调用此函数之前，isSelect必须返回true

QSqlError lastError() const;
QSqlRecord record() const;//返回包含当前查询的字段信息的QSqlRecord。如果查询指向一个有效的行，则使用该行的值填充记录。当没有活动查询时，将返回一条空记录
QSql::NumericalPrecisionPolicy numericalPrecisionPolicy() const;
QVariant lastInsertId() const;//如果数据库支持，则返回最近插入的行的对象ID。如果查询未插入任何值或数据库未报告ID，则将返回无效的QVariant。如果插入操作接触了多行，则行为未定义
QString lastQuery() const;//返回正在使用的当前查询的文本，如果没有当前查询文本，则返回空字符串
QString executedQuery() const;//返回上次成功执行的查询
QVariant value(int index) const;//返回当前记录中字段索引的值
QVariant value(const QString &name) const;
int at() const;//返回查询的当前内部位置。第一条记录位于零位。如果位置无效，函数将返回QSql:：BeforeFirstRow或QSql：：AfterLastRow，它们是特殊的负值。
int size() const;//返回结果的大小（返回的行数），如果无法确定大小或数据库不支持报告有关查询大小的信息，则返回-1。请注意，对于非SELECT语句（isSelect（）返回false），size（）将返回-1。如果查询不是活动的（isActive（）返回false），则返回-1
int numRowsAffected() const;//返回受结果的SQL语句影响的行数，如果无法确定，则返回-1。请注意，对于SELECT语句，该值是未定义的；请改用size（）。如果查询未处于活动状态，则返回-1。
...
QSqlRelation是QSqlRelationalTableModel的辅助类。有关详细信息，请参见QSqlRelationalTableModel:：setRelation（）和QSqlRelationTableModel:（relation）。

```c++
QSqlRelation(const QString &tableName, const QString &indexColumn, const QString &displayColumn);
QSqlRelation();
QString displayColumn() const;//返回表tableName（）中应显示给用户的列，而不是外键。
QString indexColumn() const;//返回外键引用的表tableName（）中的索引列
bool isValid() const;
void swap(QSqlRelation &other);
QString tableName() const;//返回外键引用的表的名称
```
...
### 10.8 QSqlRelationalDelegate

与默认委托不同，QSqlRelationalDelegate为其他表中的外键字段提供了一个组合框。要使用该类，只需在具有QSqlRelationalDelegate实例的视图上调用QAbstractItemView:：setItemDelegate（）：
...
关系表模型示例（如下所示）说明了如何将QSqlRelationalDelegate与QSqlRelationalTableModel结合使用，为表提供外键支持。

### 10.9 QSqlResult

通常，您会使用QSqlQuery而不是QSqlResult，因为QSqlQuery为特定于数据库的QSqlResult实现提供了通用包装器。如果您正在实现自己的SQL驱动程序（通过子类化QSqlDriver），则需要提供自己的QSqlResult子类，该子类实现所有纯虚拟函数和所需的其他虚拟函数。

### 10.10 QSqlQueryModel
...
**默认情况下，该模型是只读的。要使其读写，必须对其进行子类化并重新实现**setData（）和flags（）。另一种选择是使用QSqlTableModel，它提供了一个基于单个数据库表的读写模型。

### 10.11 QSqlTableModel
...
上面的代码片段从来自employee的查询SELECT*的结果集中的记录4中提取了salary字段。可以使用setFilter（）设置筛选器，也可以使用setSort（）修改排序顺序。**最后，您必须调用select()来用数据填充模型**。
表模型示例说明了如何使用QSqlTableModel作为QTableView的数据源。QSqlTableModel不直接支持外键。如果要解析外键，请使用QSqlRelationalTableModel和QSqlRelationalDelegate。

```c++
enum EditStrategy { OnFieldChange, OnRowChange, OnManualSubmit };
//对模型的所有更改都将立即应用于数据库;当用户选择其他行时，将应用对行的更改;所有更改都将缓存在模型中，直到调用submitAll（）或revertAll（）为止
...
QString tableName() const;//返回当前所选表的名称
...
virtual void revertRow(int row);//还原指定行的所有更改
...
### 10.12 QSqlRelationalTableModel

左边的屏幕截图显示了一个QTableView中的普通QSqlTableModel。外键（城市和国家）不能解析为人类可读的值。右边的屏幕截图显示了一个QSqlRelationalTableModel，其中外键被解析为人类可读的文本字符串。
...
函数调用setRelation（）在两个表之间建立关系。第一个调用指定employee表中的列2是一个外键，它与表city的字段id映射，并且视图应该向用户显示city的name字段。第二个调用对第3列执行类似的操作。
如果使用读写QSqlRelationalTableModel，则可能希望在视图上使用QSqlRelationalDelegate。与默认委托不同，QSqlRelationalDelegate为其他表中的外键字段提供了一个组合框。要使用该类，只需在具有QSqlRelationalDelegate实例的视图上调用QAbstractItemView:：setItemDelegate（）：
...
如果关系的显示列名也用作关系表中的列名，或者如果它在多个关系中用作显示列名，则它将被别名化。别名是关系的表名、显示列名和由下划线连接的唯一id（例如tablename_columnname_id）。QSqlRecord:：fieldName（）将返回带别名的列名。当检测到重复时，所有重复显示列名的出现都会产生别名，但不会对主表中的列名产生别名。别名不会影响QSqlRelation，因此QSqlRelation:：displayColumn（）将返回原始显示列名。
引用表名称是别名。别名是单词“relTblAl”和由下划线连接的相关列索引（例如relTblAl_2）。别名可用于筛选表（例如，setFilter（“relTblAl_2='Oslo'或relTblAl_3='USA'”）。
使用setData（）时，角色应始终为Qt:：EditRole，使用data（）时角色应始终是Qt::DisplayRole。
...
slot virtual void revertRow(int row) override;
```

### 10.13 QSqlRelationalDelegate
