...
class TcpFixedQuery : public QObject
{
      Q_OBJECT
public:
      explicit TcpFixedQuery(QTcpSocket * socket,QObject *parent = Q_NULLPTR);
      void connectHost(const QString& ip, uint16_t port);
      void disconnectHost();
      void fixedQuery(); // 提供给外部反复查询
private:
