...
TcpQuery::TcpQuery(QTcpSocket * socket,QObject *parent) : QObject(parent),
      mTcpSocket(socket)
{
        // 表格的设置selectedPeriod => 表格 => 基本设置 => 主窗口 => 通讯 => 串口固定查询线程 => 处理器 upSelectedPeriod
...
//                &mProcessor,SIGNAL(selectedPeriod(uint32_t, uint32_t,uint32_t,uint32_t)));

        // （2）处理器 => this（处理器同时处理2种数据都要连接）
...
//        });
        ...
        connect(mTcpSocket,static_cast<void (QTcpSocket::*)(QAbstractSocket::SocketState)>(&QTcpSocket::stateChanged),
                this,[=](QAbstractSocket::SocketState state){
                  switch (state)
                 {
                    //未连接的时候可能是错误导致的有必要发送此信号让通讯清理资源
                     case QAbstractSocket::SocketState::UnconnectedState: emit errorCode(TCPERRORSTATE::UnconnectedState);break;
