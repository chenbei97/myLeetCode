#ifndef TESTTRTRANSLATION_H
#define TESTTRTRANSLATION_H
...
QT_BEGIN_NAMESPACE
namespace Ui { class TestTRTranslation; }
QT_END_NAMESPACE

class TestTRTranslation : public QMainWindow
{
  Q_OBJECT
private:
    //用于状态栏的信息显示
    ...
    QWDialogHeaders *dlgSetHeaders=Q_NULLPTR;//设置表头文字对话框, 一次创建，exec()重复调用
    void closeEvent(QCloseEvent *event);//关闭窗口时间，可以询问是否退出
public:
  explicit TestTRTranslation(QWidget *parent = nullptr);
  ~TestTRTranslation();
...
private:
  Ui::TestTRTranslation *ui;
};
#endif // TESTTRTRANSLATION_H
