...
Communication::Communication(QWidget * parent) :QDialog(parent),mLogMultiple(30),mLogHandle(0),mLogExec(false),
      ...
      mHostName(tr("192.168.200.100")),mHostPort(30000),mTcpSocket(new QTcpSocket),mTcpQueryThread(new QThread),
      mTcpQueryWorker(new TcpQuery(mTcpSocket)),mTcpQueryTimer(new QTimer(this))
{
        ...
        initTcpConnections();
        // 通讯的日志周期 => 告知串口固定查询线程对象、TCP查询工作对象，因为这2个对象内部有数据处理器对象
//        connect(this,SIGNAL(selectedPeriod(quint32, quint32,quint32,quint32)),
//                mPortFixedQueryThread,SIGNAL(selectedPeriod(quint32,quint32,quint32,quint32)));
//        connect(this,SIGNAL(selectedPeriod(uint32_t, uint32_t,uint32_t,uint32_t)),
//                mTcpQueryWorker,SIGNAL(selectedPeriod(uint32_t, uint32_t,uint32_t,uint32_t)));
        connect(this,static_cast<void (Communication::*)(quint32,quint32,quint32,quint32)>(&Communication::selectedPeriod),this,
                [=](quint32 ms,quint32 s,quint32 m,quint32 h){//启动时该信号不会触发,所以mLogPeriodRate要事先初始化为30(9s)
                    quint32 total_ms = ms + s * 1000 + m * 60000 + h * 3600000; // 全部转为ms
                    // qDebug()<<"倍数："<<total_ms / mFixedQueryTime;
                    mLogMultiple = total_ms / mFixedQueryTime; //会自动取整
        });
}
...
void Communication::on_PortConfigureBtn()
{
      ...
      if (ret == QDialog::Accepted)
      {
          if (hasChanged(serialConfig->settings())) // 串口的新设置和之前的不一样
          {
              ...
              emit updatePortSettings(); // 通知串口设置发生了改变,信号绑定的匿名函数更新设置
          }
      }
      delete  serialConfig;
}
