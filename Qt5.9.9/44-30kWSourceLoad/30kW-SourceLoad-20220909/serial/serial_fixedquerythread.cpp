        ...
        quint32 maxAllowWriteTimeout = mQueryInterval/5; // 假设查询周期是300ms必须完成一次查询,极端情况重复了5次,故每次最多允许等待60ms
         ...
         else // 说明写入请求已经成功,可以等待回复
         {
              ...
              else // 说明读取成功
              {
                  ...
                  QByteArray responseData = mPort->readAll();
                  while (!responseData.isEmpty())
                  {
                         int currEnd = responseData.indexOf('\n'); // 可能为-1,第一个命令的截止符
                         //qDebug()<<currEnd+1;
                         if(currEnd+1 == 0)responseData.clear(); //这一步很重要,有可能回复的数据是错乱的,剩下的数据没有\n,那么这种数据可以抛弃
                         // 如果不做处理，while一直卡住,因为remove总是(0,0),那么responseData不会为空永远不会跳出
                         // 那么mPort内部缓存会累积不能在下次被读取给responseData,这样内存就会爆掉
                         QByteArray responseBytes = responseData.left(currEnd+1); // 使用left来获取,包含\n字符,此时就可以依据是否带分号来区分来自定时查询还是设定查询
                         responseData.remove(0,currEnd+1);//移除最早的回复清理缓存
                        if (responseBytes.contains(';'))  //只有定时查询查询的回复数据才带有分号
                        {
                            emit availableFixedQueryData(responseBytes);// 定时查询的数据发出去
                        }
                        else
                        {
                            emit availableSetQueryData(responseBytes,mRequestBytes,mRole); // 这里把回复的数据、设定的数据和角色一并发出去,communicate_with_fixquery.cpp中处理
                        }
                  }
                      // 只要监控界面设定值立刻就会在通讯模块的communicate_with_monitor.cpp把sendInfo信号发出并连接到这里槽函数更新mRole和mRequestBytes
                     // 之所以绕一个圈子没有让mRole/requestBytes直接作为communication的属性，是为了保证这是一个完整的事件
                     // communication：设定命令+发出sendInfo => this：更新mRole/mRequestBytes+转发回复的数据 => communication：成对拿到回复的数据、要设定的数据和角色处理比较安全
                     // 如果直接在communication设定命令+设定mRole/mRequestBytes，然后this转发回复的数据，再到communication拿到新数据根据mRole/mRequestBytes处理
                     // 可能会存在问题，因为拿到的数据和mRole/mRequestBytes并不是同时获得的
...
void SerialFixedQueryThread::on_SendInfo(SETTINGVARIABLE role,QByteArray request)
{
     mRole = role;
     mRequestBytes = request;
}
