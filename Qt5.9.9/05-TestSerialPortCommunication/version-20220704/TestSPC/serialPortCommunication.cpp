...
void SerialPortCommunication::on_volInput_valueChanged(double arg1)
{
   ...
   QString prefix = SPIC::spic(SPIC::spiCE::SOUR_VOLT_SET_VAL);
...
void SerialPortCommunication::on_curInput_valueChanged(double arg1)
{
  ...
  QString prefix = SPIC::spic(SPIC::spiCE::SOUR_CURR_SET_VAL);
  prefix += QString::asprintf("%.2f",arg1);
  const char * res = prefix.toStdString().c_str();
  qDebug()<<res;
  this->Port->write(res);
...
void SerialPortCommunication::on_powInput_valueChanged(double arg1)
{
  ...
  QString prefix = SPIC::spic(SPIC::spiCE::SOUR_POW_SET_VAL);
...
void SerialPortCommunication::on_RInput_valueChanged(double arg1)
{
  ...
  QString prefix = SPIC::spic(SPIC::spiCE::SOUR_RES_SET_VAL);
  prefix += QString::asprintf("%.2f",arg1);
  const char * res = prefix.toStdString().c_str();
  qDebug()<<res;
  this->Port->write(res);
...
void SerialPortCommunication::send_query_command()
{
  ...
  if (!this->Port->isOpen()){
      qDebug()<<time.toString("hh::mm::ss")<<"Serial port is closed";
      return;
    }

  QList<SPIC::spiCE> commands = {
    SPIC::spiCE::MEAS_VOLT_AVAE,
    SPIC::spiCE::MEAS_CURR_AVAE,
    SPIC::spiCE::MEAS_POW_AVAE,
    SPIC::spiCE::MEAS_RES_AVAE
  };

  // \n对应的HEX是OA,?是3F
  qDebug()<<time.toString("hh::mm::ss")<<"send an command";
  this->Port->write(SPIC::spic(SPIC::spiCE::MEAS_VOLT_AVAE).toStdString().c_str());
  this->Port->write("MEAS:CURR?\n");
  this->Port->write("MEAS:POW?\n");
  this->Port->write("MEAS:RES?\n");
}
void SerialPortCommunication::receive_query_result()
{
