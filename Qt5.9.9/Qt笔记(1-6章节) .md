...
mStartTableBtn = new QPushButton(tr("表格"));
mStartPlotBtn = new QPushButton(tr("绘图"));
quint32 btnRadius = 100;
mStartTableBtn->setFixedSize(QSize(btnRadius,btnRadius));
mStartPlotBtn->setFixedSize(QSize(btnRadius,btnRadius));
QString style = "border:2px solid black;border-radius:50px;background-color:#0776a9";
mStartTableBtn->setStyleSheet(style);// 设置圆形按钮
mStartPlotBtn->setStyleSheet(style);
```
...
QAbstractItemModel 类定义了项目模型必须使用的标准接口，以便能够与模型/视图架构中的其他组件进行互操作。每个项目都有一个由 QModelIndex 指定的唯一索引。可以通过模型访问的每一项数据都有一个关联的模型索引。您可以使用 index() 函数获取此模型索引。每个索引可能有一个sibling()索引；子项有一个 parent() 索引。每个项目都有与之关联的数据元素，可以通过为模型的 data() 函数指定一个角色（参见 Qt::ItemDataRole）来检索它们。可以使用 itemData() 函数同时获取所有可用角色的数据。每个角色的数据使用特定的 Qt::ItemDataRole 设置。单个角色的数据使用 setData() 单独设置，或者可以使用 setItemData() 为所有角色设置。可以使用 flags() 查询项目（参见 Qt::ItemFlag）以查看它们是否可以被选择、拖动或以其他方式操作。如果一个项目有子对象，hasChildren() 为相应的索引返回 true。
该模型对于层次结构的每个级别都有一个 rowCount() 和一个 columnCount()。可以使用 insertRows()、insertColumns()、removeRows() 和 removeColumns() 插入和删除行和列。
该模型发出信号以指示变化。例如，只要模型提供的数据项发生更改，就会发出 dataChanged() 。更改模型提供的标头会导致发出 headerDataChanged()。如果底层数据的结构发生变化，模型可以发出 layoutChanged() 以向任何附加视图指示它们应该重新显示显示的任何项目，同时考虑到新结构。可以使用 match() 函数搜索模型中可用的项目以查找特定数据。要对模型进行排序，可以使用 sort()。

此枚举描述模型更改布局的方式。

```c++
enum QAbstractItemModel::LayoutChangeHint{
    QAbstractItemModel::NoLayoutChangeHint//没有可用的提示
    QAbstractItemModel::VerticalSortHint//正在对行进行排序
    QAbstractItemModel::HorizontalSortHint//正在对列进行排序
}
```
...
```c++
// 索引操作
virtual QModelIndex index(int row, int column, const QModelIndex &parent = QModelIndex()) const = 0;//返回由给定行、列和父索引指定的模型中项的索引
bool hasIndex(int row, int column, const QModelIndex &parent = QModelIndex()) const;
virtual int rowCount(const QModelIndex &parent = QModelIndex()) const = 0;
virtual int columnCount(const QModelIndex &parent = QModelIndex()) const = 0;
virtual QModelIndex parent(const QModelIndex &index) const = 0;
virtual bool hasChildren(const QModelIndex &parent = QModelIndex()) const;
//返回索引处项的行和列的同级项，如果该位置没有同级项，则返回无效的QModelIndex
virtual QModelIndex sibling(int row, int column, const QModelIndex &index) const;
virtual QModelIndex buddy(const QModelIndex &index) const;//返回由索引表示的项的伙伴的模型索引
virtual QModelIndexList match(const QModelIndex &start, int role, const QVariant &value, int hits = 1, Qt::MatchFlags flags = Qt::MatchFlags( Qt::MatchStartsWith | Qt::MatchWrap )) const;//返回起始索引列中存储在给定角色下的数据与指定值匹配的项的索引列表
virtual bool canFetchMore(const QModelIndex &parent) const;
virtual void fetchMore(const QModelIndex &parent);//如果父级有更多可用数据，则返回true
...
// 设置指定单元格/表头数据(role,value)=(int,QVariant)
...
virtual void sort(int column, Qt::SortOrder order = Qt::AscendingOrder);//按列对模型排序
virtual QSize span(const QModelIndex &index) const;//返回由索引表示的项的行和列跨度
virtual QHash<int, QByteArray> roleNames() const;//返回模型的角色名称
//返回索引的项标志。基类实现返回启用项(ItemIsEnabled)并允许选择它(ItemIsSelectable)的标志组合
virtual Qt::ItemFlags flags(const QModelIndex &index) const;
virtual bool dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent);//处理以给定操作结束的拖放操作提供的数据
virtual bool canDropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent) const;//如果模型可以接受数据滴，则返回true
virtual QMimeData *mimeData(const QModelIndexList &indexes) const;
virtual QStringList mimeTypes() const;//返回允许的MIME类型列表
virtual Qt::DropActions supportedDragActions() const;//返回此模型中支持的拖动操作
...
// 无论何时更改标题，都会发出此信号。方向指示水平或垂直标头是否已更改
void headerDataChanged(Qt::Orientation orientation, int first, int last);
// 该信号在模型布局更改之前发出
void layoutAboutToBeChanged(const QList<QPersistentModelIndex> &parents = QList<QPersistentModelIndex> (), QAbstractItemModel::LayoutChangeHint hint = QAbstractItemModel::NoLayoutChangeHint);
// 每当模型所暴露的项目的布局发生变化时，就会发出该信号；例如当模型已被排序时
void layoutChanged(const QList<QPersistentModelIndex> &parents = QList<QPersistentModelIndex> (), QAbstractItemModel::LayoutChangeHint hint = QAbstractItemModel::NoLayoutChangeHint);
// 当调用beginResetModel()时，在模型的内部状态（例如持久模型索引）无效之前，会发出此信号
void modelAboutToBeReset();
// 当模型的内部状态（例如持久模型索引）无效后，调用endResetModel()时会发出此信号。
void modelReset();

// 公共槽函数
virtual void revert();//让模型知道它应该丢弃缓存的信息。此函数通常用于行编辑
virtual bool submit();//让模型知道它应该将缓存的信息提交到永久存储。此函数通常用于行编辑
```

##### QAbstractListModel

QAbstractListModel类提供了一个抽象模型，可以将其子类化以创建一维列表模型。
QAbstractListModel为将其数据表示为简单的非层次项目序列的模型提供了一个标准接口。它不直接使用，但必须被子类化。
简单的模型可以通过子类化这个类并实现最少数量的所需函数来创建。例如，我们可以实现一个简单的基于QStringList的只读模型，该模型向QListView小部件提供字符串列表。在这种情况下，我们只需要实现rowCount()函数来返回列表中的项目数，并实现data()函数从列表中检索项目。
由于模型表示一维结构，rowCount()函数返回模型中项目的总数。columnCount()函数是为了与所有类型的视图互操作而实现的，但默认情况下会通知视图模型只包含一列。
...
在对QAbstractListModel进行子类化时，必须提供rowCount()和data()函数的实现。行为良好的模型还提供了headerData()实现。如果您的模型在QML中使用，并且需要角色而不是roleNames()函数提供的默认角色，则必须重写它。对于可编辑列表模型，还必须提供setData()的实现，并实现flags()函数，使其返回包含Qt::ItemIsEditable的值。
注意，QAbstractListModel提供了columnCount()的默认实现，该实现通知视图该模型中只有一列项。
为可调整大小的列表式数据结构提供接口的模型可以提供insertRows()和removeRows()的实现。在实现这些函数时，重要的是调用适当的函数，以便所有连接的视图都知道任何更改：insertRows()实现必须在将新行插入数据结构之前调用beginInsertRows()。在从数据结构中删除行之前，removeRows()实现必须调用beginRemoveRows()，然后必须立即调用endRemoveRows()。
...
QAbstractProxyModel类为可以执行排序、过滤或其他数据处理任务的代理项模型提供基类。
此类定义了代理模型必须使用的标准接口，以便能够与其他模型/视图组件正确互操作。它不应该直接实例化。所有标准代理模型都源自QAbstractProxyModel类。如果您需要创建一个新的代理模型类，通常最好将一个现有的类子类化，该类提供与您想要提供的行为最接近的行为。
应该通过使用QSortFilterProxyModel或将其子类化来创建从源模型中筛选或排序数据项的代理模型。
要子类化QAbstractProxyModel，需要实现mapFromSource()和mapToSource()。mapSelectionFromSource()和mapSelectionToSource()函数仅在需要与默认行为不同的行为时才需要重新实现。
注意：如果删除了源模型或未指定源模型，则代理模型将在空占位符模型上运行。

公共函数。

```c++
// 重新实现此函数以返回代理模型中与源模型的sourceIndex相对应的模型索引
virtual QModelIndex mapFromSource(const QModelIndex &sourceIndex) const = 0;
// 重新实现此函数以返回源模型中与代理模型中的proxyIndex对应的模型索引
virtual QModelIndex mapToSource(const QModelIndex &proxyIndex) const = 0;
// 返回从指定sourceSelection映射的代理选择
...
与其他数据模型结合，提供排序和过滤功能的数据模型类。不常用。

###### QIdentityProxyModel

QIdentityProxyModel类代理其源模型未修改的QIdentity代理模型可以用于准确地转发源模型的结构，而无需排序、过滤或其他转换。这在概念上类似于恒等矩阵，其中A.I=A。
因为它不进行排序或过滤，所以这个类最适合于转换源模型的data()的代理模型。例如，可以创建一个代理模型来定义使用的字体、背景颜色或工具提示等。这样就不需要在创建模型结构的同一类中实现所有数据处理，也可以用于创建可重用的组件。不常用。

##### QAbstractTableModel

参考[Model/View Programming | Qt Widgets 5.15.14](https://doc.qt.io/qt-5/model-view-programming.html#creating-new-models)和[QAbstractTableModel Class | Qt Core 5.15.14](https://doc.qt.io/qt-5/qabstracttablemodel.html)和[QAbstractItemModel Class | Qt Core 5.15.14](https://doc.qt.io/qt-5/qabstractitemmodel.html)。

QAbstractTableModel类提供了一个抽象模型，可以将其子类化以创建表模型。
QAbstractTableModel为将数据表示为二维项目数组的模型提供了一个标准接口。它不直接使用，但必须被子类化。rowCount()和columnCount()函数返回表的维度。要检索与模型中的项相对应的模型索引，请使用index()并只提供行号和列号。

在子类化QAbstractTableModel时，必须实现rowCount()、columnCount()和data()。index()和parent()函数的默认实现由QAbstractTableModel提供。行为良好的模型也将实现headerData()。可编辑模型需要实现setData()，并实现flags()以返回包含Qt::ItemIsEditable的值。为可调整大小的数据结构提供接口的模型可以提供insertRows()、removeRows()和insertColumns()的实现。在实现这些函数时，重要的是调用适当的函数，以便所有连接的视图都知道任何更改：insertRows()实现必须在将新行插入数据结构之前调用beginInsertRows()。insertColumns()实现在将新列插入数据结构之前必须调用beginInsertColumns()。在从数据结构中删除行之前，removeRows()实现必须调用beginRemoveRows()，然后必须立即调用endRemoveRows()。在从数据结构中删除列之前，removeColumns()实现必须调用beginRemoveColumns()，然后必须立即调用endRemoveColumns()。
...
用于数据库的一个数据表的数据模型类。继承自QSqlQueryModel。

###### QSqlRelationalTableModel

数据关系模型，继承自QSqlTableModel。

以上类的说明在[](Qt笔记(7-15章节).md)可以查看。
