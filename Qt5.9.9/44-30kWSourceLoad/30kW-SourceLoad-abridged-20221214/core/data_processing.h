...
public:
      ...
      void updateSelectedPeriod(uint32_t ms, uint32_t s ,uint32_t , uint32_t h);
      void run();
private:
      QByteArray mData;
 ...
 signals:
      void fixedQuery(const QList<QByteArray>& data); // 处理后的2类数据对外发送
      void setQuery(bool ret,SETTINGVARIABLE role) ;
      void selectedPeriod(uint32_t ms, uint32_t s = 0,uint32_t m = 0, uint32_t h = 0); // 外部传入的日志设定周期
};
...
inline void ResponseProcessor::updateSelectedPeriod(uint32_t ms, uint32_t s, uint32_t, uint32_t h)
{
